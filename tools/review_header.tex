...
% Derivations in a chat transcript strike terms out rather than restating the
% line, which needs \cancel; amssymb carries the \checkmark and \mathbb that
% the same messages reach for. Lesson 4's session used \cancel sixteen times
% and stopped the build on the first one.
\usepackage{cancel}
\usepackage{amssymb}

% "Does not imply" arrives as \centernot\Rightarrow, which a chat renderer
% accepts and plain LaTeX does not define. Lesson 1's session used it twice and
% stopped the build on the first.
