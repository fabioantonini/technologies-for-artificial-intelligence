...
is learned and that is the whole rule of the day. The baseline at 0.629, because
it is what makes 0.986 mean something. And the threshold sweep at the end, where
the same model - unchanged, same weights - goes from missing no malignancy with
eleven false alarms to missing seven with none.

Tell them to run it themselves rather than watch: everything after this in the
course assumes they have a working environment, and this is the session where a
broken one shows up.
:::
...
::: notes
The frame, and the next slide opens the notebook: none of these is a coding error.
All four run exactly as written and return a number that a reviewer, a supervisor or a customer
would accept.
...
pass a review. The work is reading them.

The two cells to make sure nobody skips: the leaked selection reaching 77% on
labels generated by a coin flip, and the four deployment scenarios for the
shortcut column, where the two that raise an exception turn out to be the lucky
ones.
:::
