...
| 4 | **Two submission conventions.** Exercises 1–3 were due "at the start of Lesson N+1"; exercises 4–10 at "23:59" on the same Friday. The dates chained correctly end to end, so only the *time* differed. | **Closed, 29 August 2026: 09:00 on the Friday, at the start of the next lesson**, written the same way in the exercise, the handout, the deck, the speaker notes and `SCHEDULE.md`. The decks settled it rather than taste. Five of them open by collecting the exercise "due today" and naming the recurring mistake out loud — under 23:59 that discussion happens *before* the deadline, so a student who has not submitted hears which mistake to avoid and then submits. Exercise 10 keeps 23:59 and now says why: it is the only one with no lesson after it. Two consequences followed. Exercises 1–3 gained the YAML front matter the other seven had, so all ten PDFs carry the same title block, and "bring questions to the start of lesson N" became email, since that start is now the deadline itself. |
...
| 7 | **Four decks hand back an exercise on the day it is due.** Lessons 7 to 10 open with "Exercise N returned" — lesson 7 returns exercise 6, which is due that morning. There is no interval in which to mark it, under the old deadline or the new one. Lessons 2 to 6 do the opposite and *collect* the exercise due that day, which is consistent. | **Found 29 August 2026, left for the instructor**, because the fix is a choice about the marking cycle rather than a correction. Either those four slides become "collected", matching lessons 2–6 and keeping their content, which is about the exercise due that day; or the hand-back moves one lesson later, so lesson N returns exercise N−2 and the marking week is real, which means renumbering the four slides and rewriting their bullets around the earlier exercise. |
